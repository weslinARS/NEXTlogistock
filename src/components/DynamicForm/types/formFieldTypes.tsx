...
export type InputType =
  ...
  | 'select'
  | 'textarea'
  | 'debounced-combobox'; // <-- Nuevo tipo agregado
...
// --- 3.1 Nuevo input DebouncedCombobox ---
// Define la configuración para el combobox con debounce
export interface DebouncedComboboxConfig<O> {
  fetchOptions: (query: string) => Promise<Array<O>>;
  debounceTime?: number;
}

// Interfaz para el nuevo componente DebouncedCombobox
export interface DebouncedComboboxInputProps<T, O> extends BaseInputProps<T> {
  type: 'debounced-combobox'; // <-- Asegura que el tipo sea 'debounced-combobox'
  debouncedComboboxConfig: DebouncedComboboxConfig<O>;
}

// Factory
export function createDebouncedComboboxInput<T, O>(
  props: DebouncedComboboxInputProps<T, O>,
): DebouncedComboboxInputProps<T, O> {
  return createBaseInput(props) as DebouncedComboboxInputProps<T, O>;
}
...
// 2.2) DebouncedComboboxInputProps → tiene `debouncedComboboxConfig`
export function isDebouncedComboboxInput<T>(
  input: InputDefinition<T>,
): input is DebouncedComboboxInputProps<T, any> {
  return 'debouncedComboboxConfig' in input;
}
...
export function isBaseInput<T>(
  ...
  return (
    !isArrayInput(input) &&
    !isSelectInput(input) &&
    !isCreatableSelectInput(input) && // <-- Nuevo type guard
    !isDebouncedComboboxInput(input) && // <-- Nuevo type guard
...
export type InputDefinition<T> =
  ...
  | ReturnType<typeof createDebouncedComboboxInput<T, any>> // <-- Nuevo tipo agregado
